defmodule DungeonCaster.Session.ServerTest do
  use ExUnit.Case
  ...
  test "play_audio :ambient sets ambient track", %{sid: sid} do
    :ok = DungeonCaster.Session.Server.play_audio(sid, "music/tavern.mp3", :ambient)
    state = DungeonCaster.Session.Server.get_state(sid)
    assert state.audio_state.ambient == "music/tavern.mp3"
  end

  test "stop_audio :ambient clears ambient track", %{sid: sid} do
    DungeonCaster.Session.Server.play_audio(sid, "music/tavern.mp3", :ambient)
    :ok = DungeonCaster.Session.Server.stop_audio(sid, :ambient)
    state = DungeonCaster.Session.Server.get_state(sid)
    assert state.audio_state.ambient == nil
  end

  test "set_volume updates volume map", %{sid: sid} do
    :ok = DungeonCaster.Session.Server.set_volume(sid, %{master: 60, ambient: 70, sfx: 100})
    state = DungeonCaster.Session.Server.get_state(sid)
  ...
  test "update_hp changes hp of specific combatant", %{sid: sid} do
    ...
    DungeonCaster.Session.Server.set_initiative(sid, combatants)
    :ok = DungeonCaster.Session.Server.update_hp(sid, "c1", 3)
    state = DungeonCaster.Session.Server.get_state(sid)
  ...
  test "set_volume with 0 for master sets master to 0", %{sid: sid} do
    :ok = DungeonCaster.Session.Server.set_volume(sid, %{master: 0})
    state = DungeonCaster.Session.Server.get_state(sid)
  ...
  test "add_stroke prepends stroke to drawings", %{sid: sid} do
    ...
    :ok = DungeonCaster.Session.Server.add_stroke(sid, stroke1)
    :ok = DungeonCaster.Session.Server.add_stroke(sid, stroke2)
    state = DungeonCaster.Session.Server.get_state(sid)
  ...
  test "clear_player_drawings removes only that player's strokes", %{sid: sid} do
    ...
    DungeonCaster.Session.Server.add_stroke(sid, stroke_p1)
    DungeonCaster.Session.Server.add_stroke(sid, stroke_p2)
    :ok = DungeonCaster.Session.Server.clear_player_drawings(sid, "p1")
    state = DungeonCaster.Session.Server.get_state(sid)
  ...
  test "toggle_player_qr flips show_player_qr", %{sid: sid} do
    assert DungeonCaster.Session.Server.get_state(sid).show_player_qr == false
    :ok = DungeonCaster.Session.Server.toggle_player_qr(sid)
    assert DungeonCaster.Session.Server.get_state(sid).show_player_qr == true
    :ok = DungeonCaster.Session.Server.toggle_player_qr(sid)
    assert DungeonCaster.Session.Server.get_state(sid).show_player_qr == false
  end
end
