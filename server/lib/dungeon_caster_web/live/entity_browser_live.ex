defmodule DungeonCasterWeb.EntityBrowserLive do
  ...
  def mount(%{"type" => type}, _session, socket) do
    ...
    {:ok,
     assign(socket, type: type, entities: entities, q: "", page_title: type_copy(type).plural)}
  end
    ...
    <div class="p-6">
      <div class="flex items-center justify-between mb-4">
        ...
        <.link navigate={"/entities/#{@type}/new"} class="btn btn-primary btn-sm">
          <.icon name="hero-plus" class="size-4" /> New {type_copy(@type).singular}
        </.link>
      </div>
