defmodule CampaignToolWeb.EntityFormLive do
  ...
  def mount(%{"type" => type, "id" => id}, _session, socket) do
    ...
    socket =
      socket
      |> assign(mode: :edit, type: type, entity: entity, body: body,
                form_data: entity_to_form_data(type, entity),
                preview_tab: :edit, flash_msg: nil,
                page_title: "Edit #{entity_name(entity)}")
      |> allow_upload(:asset, accept: ~w(.jpg .jpeg .png .webp .gif), max_entries: 1,
  ...
  def mount(%{"type" => type}, _session, socket) do
    socket =
      socket
      |> assign(mode: :new, type: type, entity: nil, body: "",
  ...
  defp fields_to_db_attrs("npc", id, fields, body, file_path) do
    ...
    %{"id" => id, "name" => fields["name"] || "", "status" => fields["status"] || "alive",
      ...
      "portrait" => fields["portrait"], "stat_block_id" => fields["stat_block_id"],
  ...
  defp fields_to_db_attrs("session", id, fields, body, file_path) do
    ...
    %{"id" => id, "title" => fields["title"] || "", "session_number" => parse_int(fields["session_number"]) || 1,
      "status" => fields["status"] || "planned",
      "body_raw" => body, "body_html" => html, "file_path" => file_path, "tags" => [],
      "npc_ids" => [], "location_ids" => [], "map_ids" => [], "stat_block_ids" => [], "faction_ids" => []}
  end
  ...
  defp entity_to_form_data("npc", e) do
    %{"name" => e.name, "status" => e.status, "role" => e.role,
      "race" => e.race, "class" => e.class,
      "faction_ids" => Enum.join(e.faction_ids || [], ", "),
      "portrait" => e.portrait, "stat_block_id" => e.stat_block_id}
  end
        ...
        <div class="card bg-base-100 shadow mb-4">
          <div class="card-body gap-0 p-0">
            <div class="flex border-b border-base-300">
              <button type="button" phx-click="set_preview_tab" phx-value-tab="edit"
              ...
              <button type="button" phx-click="set_preview_tab" phx-value-tab="preview"
                      class={["px-4 py-3 text-sm font-medium border-b-2 -mb-px transition-colors",
  ...
  defp render_fields("npc", assigns) do
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <label class="form-control col-span-2">
        ...
        <input name="entity[faction_ids]" value={@form_data["faction_ids"]} class="input input-bordered"
               placeholder="e.g. thieves-guild, city-watch" />
      </label>
      <label class="form-control col-span-2">
        <div class="label"><span class="label-text">Portrait</span></div>
        <%= if @form_data["portrait"] do %>
          <img src={"/maps/assets/#{@form_data["portrait"]}"}
  ...
  defp render_fields("faction", assigns) do
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <label class="form-control col-span-2">
        ...
        <input name="entity[member_ids]" value={@form_data["member_ids"]} class="input input-bordered" />
      </label>
    </div>
    """
  end
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <label class="form-control col-span-2">
        ...
        <%= if @form_data["asset_path"] && @form_data["asset_path"] != "" do %>
          <img src={"/maps/assets/#{Path.basename(@form_data["asset_path"])}"}
  ...
  defp render_fields("session", assigns) do
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <label class="form-control">
        ...
        <input name="entity[session_number]" type="number" min="1"
               value={@form_data["session_number"]} class="input input-bordered" />
      </label>
      <label class="form-control">
        <div class="label"><span class="label-text">Status</span></div>
        <select name="entity[status]" class="select select-bordered">
